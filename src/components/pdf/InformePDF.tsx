...
function Portada({ nombre, modelo, fecha, textoResumen }: {
  nombre: string; modelo: 'genotipo' | 'neurotalento'; fecha: string; textoResumen?: string
}) {
...
function TalentPage({ talentKey, score, modelo, pageIndex }: {
  talentKey: string; score: number; modelo: 'genotipo' | 'neurotalento'; pageIndex: number
}) {
...
export function InformePDF({ modelo, nombre, scores, textoResumen, fecha }: InformePDFProps) {
  ...
  return (
    <Document>
      ...
      {ordenado.map((key, i) => (
        <TalentPage key={key} talentKey={key} score={scores[key] ?? 0} modelo={modelo} pageIndex={i + 2} />
      ))}
    </Document>
  )
}
