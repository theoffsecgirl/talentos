  ...
  return (
    <Svg width={300} height={300} viewBox="0 0 300 300">
      ...
      {keys.map((key, i) => {
        ...
        return (
          <Path key={key} d={`M ${cx} ${cy} L ${x1} ${y1} A ${rv} ${rv} 0 0 1 ${x2} ${y2} Z`}
            fill={color} fillOpacity={0.75} stroke={color} strokeWidth={0.4} />
        )
      })}
      {keys.map((_, i) => {
        const a = toRad(i)
        return <Path key={i} d={`M ${cx} ${cy} L ${cx + r * Math.cos(a)} ${cy + r * Math.sin(a)}`} stroke={BG} strokeWidth={1.5} />
      })}
